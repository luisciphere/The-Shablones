	...
	var init = function (settings) {
		...
		$li.add(cloned.filter('.has_child')).each(function () {
			...
			if (!childIconLen && settings.hasChildIcon == '1') {
				var childIconTag = $('<em class="has-child-icon"><strong></strong></em>');
			...
			function showSub() {
				...
				var childIcon = $a.find('.has-child-icon strong');
...
$(function () {
	...
	$('.accordion-wr').each(function () {
		$(this).wm_accordion({
			closeOther: parseInt($(this).data('setting-close')),
			speed: parseInt($(this).data('setting-speed'))
		});
	});

});
(function ($) {
	jQuery.fn.wm_accordion = function (options) {
		...
		var make = function () {
			var $ul = $(this);
			$ul.find('.accordion-item-title').each(function () {
				var $clicked_but;
				if ($(this).find('a').length) {
					$clicked_but = $(this).find('.icon');
				} else {
					$clicked_but = $(this);
				}

				$clicked_but.click(function () {
					var $clicked_li = $(this).closest('.accordion-item');
					var $acTitle = $(this).closest('.accordion-item-title');
					var $acBody = $acTitle.next();

					if (options.closeOther) {
						$ul.find('li.opened').not($clicked_li).each(function () {
							$(this).find('.icon').removeClass('active');
							$(this).removeClass('opened');
							$(this).find('.accordion-item-drop').slideUp(options.speed);
						});
					}

					if (!$acBody.is(':animated')) {
						$acTitle.find('.icon').toggleClass('active');
						$clicked_li.toggleClass('opened');
						$acBody.slideToggle(options.speed);
					}
				});
			});
		};
...
$(function(){
	if ($(window).width()<1200){
     var nextDivs =  $('.block-2').siblings();
        nextDivs.click(function(){
            $('.menu-7 .menu-scroll > ul ul').hide()
        });
